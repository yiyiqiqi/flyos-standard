/**
 * @file flyos_port.c
 * @brief STM32F4的FlyOS移植层实现
 * @version 1.0.0
 * @author  yiyi
 * @date    2024-6-21
 * @details 概述
 *              本文件实现STM32F4(ARM Cortex-M4)的FlyOS移植层代码。
 *              包括临界区管理、任务栈初始化、移植层硬件
 */
...
void flyos_port_init(void)
{
	...
	/* 如果配置了FPU，则启用 */
#if FLYOS_FPU_ENABLE && (__FPU_PRESENT == 1) && (__FPU_USED == 1)
	SCB->CPACR |= (0xFUL << 20); /* CP10/CP11完全访问 */
	__DSB();
	__ISB();
#endif
...
void flyos_port_start_scheduler(void)
{
	...
	/* SysTick寄存器 */
	SysTick->VAL = 0UL;                  /* 清除当前值 */
	SysTick->LOAD = ulReloadValue;       /* 设置重载值 */
...
#endif /* FLYOS_PLATFORM == FLYOS_PLATFORM_STM32F4 */
